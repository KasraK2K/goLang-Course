...
func Mutability() {
	...
	/*
		...
		// As you can see the total amount of songs on the article variable’s value wasn’t changed. To mutate the passed value, we need to pass it by reference, using a pointer.
		articleR := &Artist{Name: "Kasra", Genre: "Rock", Songs: 42}
		fmt.Printf("%s released their %dth song\n", articleR.Name, passByRefrence(articleR))
...
func passByRefrence(a *Artist) int {
	a.Songs++
	return a.Songs
}
